...
parser.add_argument("-m", "--method", metavar="method", dest="method",
	help="Method to perform clustering. Currently implemented methods are 'k-means' (default) or bayesian mixture models ('bayes')",
	default="ward")
...
parser.add_argument("-t", metavar="temp_filename", dest = "temp",
	help="Name (without extension) of the output csv file with extension. Default is temp.csv.",
	default = "temp.csv")
...
if __name__ == '__main__':
	...
	try: 
		df=pd.read_csv(args.temp, sep=',')
		df = [a.tolist() for a in df.sort_values(by='ZincID').to_numpy()]
		zinc_list =  [a[0] for a in df]
	...
	for line in out_data:
		...
		potential_idx = bisect.bisect_left(zinc_list, zinc_id)

		if potential_idx < len(zinc_list) and zinc_list[potential_idx] == zinc_id:
			curr_line = ",".join([str(a) for a in df[potential_idx]])
			pca_data.append(curr_line + "\n")
		else:
		...
		if args.verbose: 
			...
			print(f"On ligand {n:n} of {len(out_data):n}. {toc - tic:0.3f} seconds have elapsed. ({((toc - tic)/n):0.5f} s/file)", end='\r')
		if n > args.n and args.n > 0:
			break
	print("\n")


	file_out = open(args.temp, "w")
	...
	if args.verbose:
		...
		verbose_int = 2
		print(f"Starting clustering  with {args.iter:n} iterations.")
	else:
		verbose_int = 0
	...
	tic = time.perf_counter()
	method = None
	if args.method.lower() in ['bayes', 'bayesiangaussianmixture', 'bayesian'] :
		if args.verbose: print(f"Beginning Bayesian Gaussian Mixture Modelling...")
		cmpd_groups = mixture.BayesianGaussianMixture(n_components=args.components,
						random_state = 32,
		...
		method = "bayes"
	elif args.method.lower() in ['gaussian', 'mixture', 'gm', 'gaussianmixture'] :
		if args.verbose: print(f"Beginning Gaussian Mixture Modelling...")
		cmpd_groups = mixture.GaussianMixture(n_components=args.components,
                        covariance_type='full',
                        max_iter=2000,
                        tol=0.00001,
                        n_init=args.iter,
                        verbose=verbose_int,
                        verbose_interval = 50).fit(X)
		method = "gm"
	elif args.method.lower() in ['minibatch', 'minibatchkmeans'] :
		if args.verbose: print(f"Beginning minibatch k-means clustering...")
		cmpd_groups = MiniBatchKMeans(n_clusters=args.components, 
						random_state=32,
						n_init=args.components*args.iter,
						max_iter=500,
						verbose=0).fit(X)
		method = "minibatch"
	elif args.method.lower() in ['ward', 'agglomerativeclustering', 'agglomerative', 'ac', 'agg'] :
		if args.verbose: print(f"Beginning Ward clustering...")
		cmpd_groups = AgglomerativeClustering(n_clusters=args.components).fit_predict(X)
		method = "ward"
	else:
		if args.verbose: print(f"Beginning k-means clustering...")
		cmpd_groups = KMeans(n_clusters=args.components, 
						random_state=32,
						n_init=args.iter,
						max_iter=500,
						verbose=verbose_int).fit(X)
		method = "kmeans"
	...
	tic = time.perf_counter()
	if method in ["ward"]:
		df['group'] = cmpd_groups + 1
	else:
		df['group'] = cmpd_groups.predict(X)+1
	toc = time.perf_counter()
	...
	group_file.close()
	# print(groups.max())
	# groups = cmpd_groups.predict(X)+1
